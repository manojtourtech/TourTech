...
let deleteAllChats = () => {
  realm.write(() => {
    ...
    realm.delete(getAllMessage());
  });
};

// Remove Single chats from Realm database
let deleteChats = item => {
  const data = getChatById(item.recipientId);
  const message = getMessageById(item.recipientId);
  realm.write(() => {
